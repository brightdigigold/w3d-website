...
const Timer: React.FC = () => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-4 py-1 bg-yellow-50 2xl:justify-center gap-2 pr-4 2xl:pr-4 mt-3">
        {/* <img src="/alarm-filled.png" className="h-2 sm:h-4" /> */}
        <p className=" text-xs sm:text-sm text-black">₹ 6536</p>
        {metalType === "gold" ? (
          <p className="text-xs sm:text-sm text-black">
            <span className="hidden sm:inline"></span> expire in :
            {formatTime(time)}
          </p>
        ) : (
          <p className="text-xs sm:text-sm text-black">
